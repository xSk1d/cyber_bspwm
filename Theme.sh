...
BORDER_WIDTH="1"	NORMAL_BC="#fb007a"		FOCUSED_BC="#fb007a"
...
launch_theme() {
	...
	for mon in $(polybar --list-monitors | cut -d":" -f1); do
		...
		MONITOR=$mon polybar -q main2 -c "${HOME}"/.config/bspwm/rices/"${RICE}"/config.ini &
		MONITOR=$mon polybar -q main3 -c "${HOME}"/.config/bspwm/rices/"${RICE}"/config.ini &
	done
}
